    ...
    try:
        ...
        try:
            await app.send_message(
                chat_id=chat_id,
                text="<b>❌ Error sending video. Please try again later.</b>"
            )
        except Exception:
            pass
        return None, None
...
async def debug_handler(client: Client, message: Message):
    ...
    text_preview = (message.text or "")[:80] if message.text else "[no text]"
    logger.info(f"📩 RECEIVED from {user_id}: text='{text_preview}'")
...
async def fallback_text_handler(client: Client, message: Message):
    ...
    user = message.from_user
    logger.info(f"💬 Text from {user.id}: {(message.text or '')[:50]}")
...
async def start_bot():
    ...
    try:
        await app.start()
        logger.info(f"✅ Bot started! Username: @{app.me.username if app.me else 'unknown'}")
        logger.info("🤖 Bot is now online and listening for updates...")

        # Use pyrogram's idle() for proper update polling
        await idle()
